...
int main() {
    ...
    int N, K, temp = 0, ret = 0;

    cin >> N;

    std::vector<std::pair<int,int>> sequence(N);
    std::vector<std::pair<int, int>> redefinesequence(N);
    std::vector<std::pair<int,int>> distance(N);
    ...
    {
        distance[i].first = i;
        distance[i].second = sequence[i].first - sequence[i].second;
    }
    redefinesequence = sequence;
    sort(redefinesequence.begin(), redefinesequence.end());
    sort(distance.begin(), distance.end());


    for (int i = 0 ; i < N; i++)
    {
        if (redefinesequence[i].first - redefinesequence[i].second == distance[i].second)
        {
        
        }
        if (redefinesequence[i].second - temp >= 0)
        {
            ret++;
            temp = redefinesequence[i].first;
        }
    }
    cout << ret;
